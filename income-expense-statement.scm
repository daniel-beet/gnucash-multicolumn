...
;; 
 ;; section 3 of 4 to add gnctimeperiod-utilities
	 ...
	 ; change periodoptions and add-option to add to other .scm files
(let ((periodoptions options))
	(add-option
...
;		(op-val 	 (get-option the_tab "Scale Results"))
;		(scale-num-val     (get-option the_tab "Scale Number Option"))
		...
		(start-date-tp    (if (equal? whichperiod-val 'customdates )
							cust-start-date-tp
							(car datelist)))
							
					
		(end-date-tp 	 (if (equal? whichperiod-val 'customdates )
							cust-end-date-tp
							(cadr datelist)))
...
;;
  ...
  ;; change following lines for gnctimeperiod-utilities
		  (start-date-printable 	start-date-tp )
	...
	;; change to 1 "a" to see variables , use 1 "a" to hide variables
	(if (equal? 1 "a")
	(begin
...
;		(gnc:html-markup-p
 ;        (gnc:html-markup/format
 ;         (_ "The operand is %s.") 
 ;         (gnc:html-markup-b op-val )))
;				 
;		  (gnc:html-markup-p
 ;        (gnc:html-markup/format 
 ;         (_ "The scale number option is %s.")
		...
		(gnc:html-markup-p
         (gnc:html-markup/format
          (_ "The period end is %s.") 
			(gnc:html-markup-b (number->string (car end-date-tp)))))
...
;; show any imbalances if option choosen	  
	...
	(begin	
	  (let* (	   
	   (count 0)
	   (imbalance-val "")
	   (accounts-imbalance (gnc:accounts-imbalance-or-orphan start-date-tp end-date-tp))
